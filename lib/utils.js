...
export const createQueryObject = (req, searchFilter) => {
  ...
  //query for search
  if (search || search?.length || id) {
    filter = {
      where: searchFilter(search, id).where,
    };
  }

  // // query for date range
  if (fromDate || toDate) {
    // if (path.includes("/payrolls")) {
    //   filter = {
    //     where: {
    //       AND: [
    //         {
  ...
  if (payrollGroups || payrollGroups?.length) {
    if (!path.includes("/payrolls/groups")) {
      filter = {
        where: {
          OR: [
    ...
    if (path.includes("/payrolls/groups")) {
      filter = {
        where: {
          OR: [
            ...payrollGroups.map((payrollGroup) => ({
              name: {
                equals: payrollGroup,
                mode: "insensitive",
              },
...
export const timeCard = (search, id) => ({
...
export const allTimeCard = (search, id) => ({
  where: {
    id: id,
  },
});
...
export const receipt = (search, id) => {
  return {
    where: {
      payrollId: id,
    },
